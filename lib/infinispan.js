...
(function() {
  ...
  var Connection = function(p, logger) {
    ...
    var bytebuf = utils.byteBuffer(0);

    var DECODE_HEADER = f.actions(p.decodeHeader(), codec.all);
    ...
    return {
      connect: function(client, host, port) {
        return new Promise(function (succeed, fail) {
          ...
          _client.on('data', function(data) {
            // TODO: Deal with partial responses
            try {
              bytebuf = bytebuf.append(data);
            } catch(ex) {
              logger.error('Error buffering, message id unknown:', ex);
              throw ex;
            }
            ...
            while (!bytebuf.isEmpty()) {
              ...
              try {
                if (p.hasError(header)) {
                  var decodedError = p.decodeError(bytebuf);
                  var err = _.first(decodedError);
                  bytebuf = _.last(decodedError);
                  throw new Error(err);
                } else {
                  var decoder = promise.decoder;
                  logger.tracef('Call decode for request(msgId=%d)', msgId);
                  if (f.existy(decoder)) {
                    var decodedResult = decoder(header, bytebuf);
                    var result = _.first(decodedResult);
                    bytebuf = _.last(decodedResult);
                    logger.tracef('Complete promise for request(msgId=%d) with %s', msgId, result);
                    promise.success(result);
                  } else {
                    logger.tracef('Complete promise for request(msgId=%d) with empty', msgId);
                ...
                bytebuf = bytebuf.skip(); // If more data present, skip data read
                logger.tracef("After decoding request(msgId=%d), buffer contains %s", msgId, bytebuf);

                //if (!bytebuf.isEmpty())
                //if (bytebuf.buf.length > bytebuf.offset) {
                //  bytebuf.buf = bytebuf.buf.slice(bytebuf.offset);
                //  bytebuf.offset = 0
                //}
              }
            }
      ...
      write: function(id, bytebuf, decoder) {
        return new Promise(function (fulfill, reject) {
          logger.tracef('Write buffer(msgId=%d)', id);
          _client.write(bytebuf.asBuffer());
  ...
  var Client = function(host, port, options) {
    ...
    function context(size) {
      return {bytebuf: utils.byteBuffer(size), id: msgIdCounter.incr()};
      //return {buf: new Buffer(size), offset: 0, id: msgIdCounter.incr()};
    }

    function write(id, bytebuf, decoder) {
      return connect.write(id, bytebuf, decoder);
    }

    function encode(ctx, header, body) {
      var acts = f.existy(body) ? f.cat(header(ctx.id), body()) : header(ctx.id);
      return f.actions(acts, codec.trim)(ctx.bytebuf);
    }

    function future(ctx, op, body, decoder, opts) {
      var encodedByteBuf = encode(ctx, p.encodeHeader(op, opts), body);
      return write(ctx.id, encodedByteBuf, decoder);
    }
    ...
    return {
      ...
      put: function(k, v, opts) {
        ...
        var decoder = p.decodePrevOrElse(opts, p.hasSuccess, p.constant(undefined));
      ...
      replace: function(k, v, opts) {
        ...
        logger.debugl(function() { return ['Invoke replace(msgId=%d,key=%s,value=%s,opts=%s)',
                                           ctx.id, k, v, JSON.stringify(opts)]; });
        var decoder = p.decodePrevOrElse(opts, p.hasPrevious, p.hasSuccess);
