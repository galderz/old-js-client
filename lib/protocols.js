...
(function() {
  ...
  var DecodeMixin = (function() {
    var DECODE_OBJECT = f.actions([codec.decodeObject()], codec.all);
    var DECODE_VERSIONED = f.actions([codec.decodeBytes(8), codec.decodeObject()], codec.all);
    var DECODE_VINT = f.actions([codec.decodeVInt()], codec.all);
    var DECODE_PAIR = f.actions([codec.decodeObject(), codec.decodeObject()], codec.all);
    var DECODE_UBYTE = f.actions([codec.decodeUByte()], codec.all);
    ...
    function decodePrev(bytebuf) {
      var decoded = DECODE_OBJECT(bytebuf);
      var prev = _.first(decoded);
      var state = _.last(decoded);
      return _.isEmpty(prev) ? [undefined, state] : [prev, state];
    }
    ...
    return {
      ...
      decodeValue: function(header, bytebuf) {
        return isSuccess(status(header)) ? DECODE_OBJECT(bytebuf) : [undefined, bytebuf];
      },
      ...
      decodePrevOrElse: function(opts, cond, orElse) {
        ...
        return function(header, bytebuf) {
          if (shouldReturnPrev) return cond(header) ? decodePrev(bytebuf) : [undefined, bytebuf];
          return orElse(header, bytebuf);
        }
      },
      ...
      decodeError: function (bytebuf) {
        return DECODE_OBJECT(bytebuf);
        //throw new Error(DECODE_OBJECT(bytebuf));
      },
      getMsgId: function(header) {
        return header[1];
      },
      hasSuccess: function(header, bytebuf) {
        return [isSuccess(status(header)), bytebuf];
      },
      hasNotExecuted: function(header, bytebuf) {
